...
const SellerPanel = () => {
  ...
  const fetchCustomerAnalytics = async () => {
    try {
      ...
      if (response.success) {
        ...
        setCustomerAnalyticsData(response.data);
      } else {
        console.warn('Failed to load customer analytics:', response.message);
  ...
  // Mock customer analytics data for seller dashboard (fallback)
  const mockSellerCustomerAnalytics = {
  ...
  // Use real recent bookings from backend or fallback to mock data
  const recentBookings = dashboardData?.recentBookings || [];
  
  const recentActivity = recentBookings.length > 0 
    ? recentBookings.slice(0, 6).map((booking, index) => ({
        ...
        type: "booking",
        message: `${booking.service} - ₹${booking.amount}`,
                ...
                <div className="grid lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    <CustomerAnalyticsCard
                      ...
                      data={customerAnalyticsData || mockSellerCustomerAnalytics}
