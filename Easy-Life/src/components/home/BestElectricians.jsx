...
const BestElectricians = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = useState(true);
  const [itemsPerSlide, setItemsPerSlide] = useState(3);
  const navigate = useNavigate();

  // Filter electricians from businesses
  const electricians = businesses.filter(
    (business) =>
      business.category === "services" &&
      (business.subcategory === "electrician" ||
        business.name.toLowerCase().includes("electric") ||
        business.services?.some((service) =>
          service.toLowerCase().includes("electric")
        ) ||
        business.description.toLowerCase().includes("electric"))
  );
  ...
  useEffect(() => {
    if (!isAutoPlaying || electricians.length <= itemsPerSlide) return;

    const interval = setInterval(() => {
      setCurrentIndex(
        (prevIndex) =>
          (prevIndex + 1) % Math.ceil(electricians.length / itemsPerSlide)
      );
    }, 4000);

    return () => clearInterval(interval);
  }, [isAutoPlaying, itemsPerSlide, electricians.length]);
  ...
  const prevSlide = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0
        ? Math.ceil(electricians.length / itemsPerSlide) - 1
        : prevIndex - 1
    );
  };
  ...
  const handleViewAll = () => {
    navigate("/listings?category=services&subcategory=electrician");
  };

  const getCurrentSlideBusinesses = () => {
    const startIndex = currentIndex * itemsPerSlide;
    return electricians.slice(startIndex, startIndex + itemsPerSlide);
  };

  if (electricians.length === 0) {
    return null; // Don't render if no electricians found
  }
    ...
    <section className="py-6 sm:py-8 lg:py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center justify-center mb-4">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900">
              Best Electricians in Gangtok
            </h2>
          </div>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Trusted electrical services for your home and business needs
          </p>
        </motion.div>
        ...
        <div className="relative">
          ...
          {/* Navigation Arrows */}
          {electricians.length > itemsPerSlide && (
            <>
              <button
                onClick={prevSlide}
          ...
          {electricians.length > itemsPerSlide && (
            <div className="flex justify-center mt-8 space-x-2">
              {Array.from(
                { length: Math.ceil(electricians.length / itemsPerSlide) },
                (_, i) => (
                  <button
                    key={i}
...
export default BestElectricians;
